//######################################################################################################## 
//#                                                                                                    #\\
//#                               Global Surface UHI Explorer Version 3                                #\\
//#                                                                                                    #\\
//########################################################################################################


// date: 2018-11-29 (updated: 2019-06-07)
...
var intro = ui.Panel([
  ...
  ui.Label({
    value:'This app displays surface urban heat islands (UHI) at a global scale. Use the search bar or pan over the map to find your urban cluster of interest. Click anywhere within the boundary of an urban cluster, and the cluster mean statistics will be listed in the space below. You also have the option to subset and download the gridded surface UHI intensity rasters for the selected urban cluster at the bottom of the panel.',
...
panel.add(intro);
 ...
 var exportimagefun = function () {
              var export_annual = UHI_yearly_1km.unmask(-999).clip(roi.geometry()).getDownloadURL({
                      ...
                      crs: 'EPSG:4326', 
                      scale: 300})
             var export_summer = Winter_UHI_yearly_1km.unmask(-999).clip(roi.geometry()).getDownloadURL({
                      region: JSON.stringify(roi.geometry().bounds().getInfo()),
                      name: 'Summertime_gridded_UHI_data', 
                      crs: 'EPSG:4326', 
                      scale: 300})
              var export_winter = Summer_UHI_yearly_1km.unmask(-999).clip(roi.geometry()).getDownloadURL({
                      region: JSON.stringify(roi.geometry().bounds().getInfo()),
...
var UHI_yearly_1km=ee.Image('users/yceo/UHI_yearly_1km_vfF_v3')
...
// Select images from a collection with a silder.
var Summer_UHI_yearly_1km=ee.Image('users/yceo/Summer_UHI_yearly_1km_vfF_v3');
...
// Select images from a collection with a silder.
var Winter_UHI_yearly_1km=ee.Image('users/yceo/Winter_UHI_yearly_1km_vfF_v3')
